...
# Logic: Zero math. Pure rendering of absolute outputs.
...
if 'a7do_brain' not in st.session_state:
    ...
    if missing:
        st.error(f"CRITICAL BOOT FAILURE: Missing files {missing}. Please check paths.")
        st.stop()

    try:
        # Initialize the specific biological objects based on the correct file classes
    ...
    if PLOTLY_AVAILABLE:
        ...
        for b_id, b_data in bones.items():
            ...
            fig.add_trace(go.Scatter3d(
                x=[px, dx], y=[pz, dz], z=[py, dy], # Swap Y/Z for upright viewing
                mode='lines+markers',
